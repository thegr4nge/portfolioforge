...
def test_buy_adds_lot_to_queue() -> None:
    ...
    lot = _lot("VAS.AX", DATE_A, 100.0, Decimal("9000.0"))
...
def test_sell_whole_lot_returns_disposed() -> None:
    ...
    assert d.cost_basis_aud == pytest.approx(Decimal("9000.0"))
    # No open lots remain
...
def test_sell_partial_lot() -> None:
    ledger = CostBasisLedger()
    ledger.buy("VAS.AX", _lot("VAS.AX", DATE_A, 100.0, Decimal("90.0")))
    ...
    assert d.cost_basis_aud == pytest.approx(Decimal("54.0"))  # 60% of $90

    # Remaining lot
    remaining = ledger._lots["VAS.AX"]
    assert len(remaining) == 1
    assert remaining[0].quantity == pytest.approx(40.0)
    assert remaining[0].cost_basis_aud == pytest.approx(Decimal("36.0"))  # 40% of $90
...
def test_sell_multiple_lots_fifo() -> None:
    ...
    """
    ledger = CostBasisLedger()
    ledger.buy("VAS.AX", _lot("VAS.AX", DATE_A, 100.0, Decimal("9000.0")))  # lot 1 (older)
    ledger.buy("VAS.AX", _lot("VAS.AX", DATE_B, 100.0, Decimal("6000.0")))  # lot 2 (newer)
    ...
    assert remaining[0].cost_basis_aud == pytest.approx(Decimal("6000.0"))
...
def test_sell_across_two_lots() -> None:
    ...
    ledger = CostBasisLedger()
    ledger.buy("VAS.AX", _lot("VAS.AX", DATE_A, 50.0, Decimal("50.0")))  # lot 1: $1/share
    ledger.buy("VAS.AX", _lot("VAS.AX", DATE_B, 50.0, Decimal("60.0")))  # lot 2: $1.20/share
    ...
    assert d1.cost_basis_aud == pytest.approx(Decimal("50.0"))
    ...
    assert d2.cost_basis_aud == pytest.approx(Decimal("36.0"))
    ...
    assert remaining[0].cost_basis_aud == pytest.approx(Decimal("24.0"))
...
def test_sell_exceeds_position_raises() -> None:
    ledger = CostBasisLedger()
    ledger.buy("VAS.AX", _lot("VAS.AX", DATE_A, 100.0, Decimal("9000.0")))
...
def test_cost_basis_usd_none_for_aud_tickers() -> None:
    ...
    ledger = CostBasisLedger()
    lot = _lot("VAS.AX", DATE_A, 100.0, Decimal("9000.0"), cost_basis_usd=None)
    ledger.buy("VAS.AX", lot)
...
def test_multiple_tickers_independent() -> None:
    ...
    ledger = CostBasisLedger()
    ledger.buy("VAS.AX", _lot("VAS.AX", DATE_A, 100.0, Decimal("9000.0")))
    ledger.buy("VGS.AX", _lot("VGS.AX", DATE_A, 50.0, Decimal("5000.0")))
...
# ---------------------------------------------------------------------------
# Test 10 -- OpenLot cost_basis_aud is Decimal (HARD-06)
# ---------------------------------------------------------------------------


def test_open_lot_cost_basis_is_decimal() -> None:
    """OpenLot.cost_basis_aud must be an instance of Decimal after migration."""
    lot = OpenLot(
        ticker="VAS.AX",
        acquired_date=DATE_A,
        quantity=100.0,
        cost_basis_aud=Decimal("5000.0"),
        cost_basis_usd=None,
    )
    assert isinstance(lot.cost_basis_aud, Decimal)


# ---------------------------------------------------------------------------
# Test 11 -- DisposedLot cost_basis_aud is Decimal (HARD-06)
# ---------------------------------------------------------------------------


def test_disposed_lot_cost_basis_is_decimal() -> None:
    """DisposedLot.cost_basis_aud must be an instance of Decimal after migration."""
    lot = DisposedLot(
        ticker="VAS.AX",
        acquired_date=DATE_A,
        disposed_date=DATE_C,
        quantity=100.0,
        cost_basis_usd=None,
        cost_basis_aud=Decimal("4000.0"),
        proceeds_usd=None,
        proceeds_aud=5000.0,
        gain_aud=1000.0,
...
# ---------------------------------------------------------------------------
# Test 12 -- ledger.sell() returns DisposedLot with Decimal cost_basis (HARD-06)
# ---------------------------------------------------------------------------


def test_sell_returns_decimal_cost_basis() -> None:
    """ledger.sell() must return DisposedLot with cost_basis_aud as Decimal."""
    ledger = CostBasisLedger()
    ledger.buy("VAS.AX", _lot("VAS.AX", DATE_A, 100.0, Decimal("9000.0")))
    ...
    assert len(disposed) == 1
    assert isinstance(disposed[0].cost_basis_aud, Decimal)


# ---------------------------------------------------------------------------
# Test 13 -- partial lot split preserves Decimal in both parts (HARD-06)
# ---------------------------------------------------------------------------


def test_partial_lot_split_preserves_decimal() -> None:
    """Partial sell must produce Decimal cost_basis in both disposed and remaining lots."""
    ledger = CostBasisLedger()
    ledger.buy("VAS.AX", _lot("VAS.AX", DATE_A, 100.0, Decimal("9000.0")))

    disposed = ledger.sell("VAS.AX", 60.0, DATE_C)

    # Disposed lot must have Decimal cost_basis_aud
    assert len(disposed) == 1
    assert isinstance(disposed[0].cost_basis_aud, Decimal)

    # Remaining open lot must also have Decimal cost_basis_aud
    remaining = ledger._lots["VAS.AX"]
    assert len(remaining) == 1
    assert isinstance(remaining[0].cost_basis_aud, Decimal)
